...
class UserModelIsFollowingTestCase(UserModelTestCase):
    ...
    def test_is_following_and_is_followed_by_after_unfollow(self):
        u1 = db.session.get(User, self.u1_id)
        u2 = db.session.get(User, self.u2_id)
...
class UserModelSignupTestCase(UserModelTestCase):
    def test_user_signup(self):
        ...
        user = dbx(q_user).scalars().one()

        self.assertEqual(user.id, u3.id)
        self.assertIn('$2b$12', user.password)
        self.assertEqual(user.username, "u3")
        self.assertEqual(user.email, "u3@email.com")
...
class UserModelAuthenticateTestCase(UserModelTestCase):
    ...
    def test_authenticate_bad_password(self):
        self.assertFalse(User.authenticate("u1", "password123"))
...
class UserModelLikeTestCase(UserModelTestCase):
    ...
    def test_liked_messages_ids_property(self):
        ...
        self.assertIn(self.m1_id, u1.liked_messages_ids)
